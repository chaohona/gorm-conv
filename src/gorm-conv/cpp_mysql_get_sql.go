...
func CPPFieldsMapPackGetSQL_ForTables_DefineSQL(table TableInfo) (string, int) {
	var DefineSQL string = "#define " + strings.ToUpper(table.Name) + "GETSQL \"select * from "
...
func CPPFieldsMapPackGetSQL_ForTables(games []XmlCfg, f *os.File) int {
	for _, game := range games {
		for _, table := range game.DB.TableList {
			...
			f.WriteString("(MYSQL* mysql, const GORM_PB_GET_REQ* pMsg, GORM_MemPoolData *&pReqData)\n")
			f.WriteString("{\n")
