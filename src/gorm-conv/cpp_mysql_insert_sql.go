...
func CPPFieldsMapPackInsertSQL_ForTables(games []XmlCfg, f *os.File) int {
	for _, game := range games {
		for _, table := range game.DB.TableList {
			...
			f.WriteString("(MYSQL* mysql, const GORM_PB_INSERT_REQ* pMsg, GORM_MemPoolData *&pReqData)\n")
			f.WriteString("{\n")
			...
			f.WriteString("        return GORM_PackInsertSQL" + BigTable + "_One(mysql, table_" + table.Name + ", pReqData);\n")
			f.WriteString("    }\n")
			f.WriteString("    return GORM_OK;\n")
			f.WriteString("}\n")
		}
