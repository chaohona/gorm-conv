...
func CPPFieldsMapPackIncreaseSQL_ForTables_WhereSQL(table TableInfo, f *os.File) int {
	...
	f.WriteString("    iWhereLen = snprintf(szWhereBegin, iWhereLen, " + strings.ToUpper(table.Name) + "INCREASEWHERESQL ")
...
func CPPFieldsMapPackIncreaseSQL_ForTables(games []XmlCfg, f *os.File) int {
	for _, game := range games {
		for _, table := range game.DB.TableList {
			...
			f.WriteString("(MYSQL* mysql, const GORM_PB_INCREASE_REQ* pMsg, GORM_MemPoolData *&pReqData)\n")
			f.WriteString("{\n")
